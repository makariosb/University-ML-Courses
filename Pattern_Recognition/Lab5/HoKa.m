function [Rc, Rep, w] = HoKa(x1, x2, Lr, MaxRep)
    ...
    while(i < MaxRep)
    %disp(sprintf( 'Step %d\n', i )) ;
    %disp(sprintf( 'Classification Score: %7.4f%%\n', ( 100 * NoGreatValMat( b1, 0.0 ) ) / ( rows(b1) * columns(b1)) )) ;
    if (GreatValMat(b1, 0.0) == 1)
        %disp(sprintf( 'Linear Separation of classes in %d repetitions\n', i )) ;
        Rc = Rep;
        return;
    end
...
%disp(sprintf( 'Original Ho-Kasyap not convergence in %d repetitions\n', i )) ;
%disp(sprintf( 'Classification Score: %7.4f%%\n', ( 100 * NoGreatValMat( b1, 0.0 ) ) / ( rows(b1) * columns(b1)) ) );
